...
bool compareJSON(cJSON *a, cJSON *b)
{
...
int readMachineMessages(cJSON *datum)
{
  OPMSG message;
  ...
  if (message.datano != -1)
  {
    ...
    cJSON *num_datum = cJSON_CreateNumber(message.datano);
    cJSON_AddItemToObject(message_datum, "num", num_datum);

    cJSON *text_datum = cJSON_CreateString(message.data);
...
int readMachineStatus(cJSON *datum)
{
  ...
  */
  ...
  char *execution = MACHINE_EXECUTION[(status.run == 3 || status.run == 4) ? 0 : (status.run == 2 || status.motion == 2 || status.mstb != 0) ? 1 : status.run == 0 ? 2 : 3];
  cJSON *execution_datum = cJSON_CreateString(execution);
  cJSON_AddItemToObject(datum, "execution", execution_datum);

  char *mode = MACHINE_MODE[(status.aut == 5 || status.aut == 6) ? 0 : (status.aut == 0 || status.aut == 3) ? 1 : 2];
  cJSON *mode_datum = cJSON_CreateString(mode);
  cJSON_AddItemToObject(datum, "mode", mode_datum);

  char *estop = MACHINE_ESTOP[status.emergency == 1 ? 0 : 1];
  ...
  cJSON *aut_datum = cJSON_CreateNumber(status.aut);
  cJSON_AddItemToObject(raw_datum, "aut", aut_datum);

  cJSON *run_datum = cJSON_CreateNumber(status.run);
  cJSON_AddItemToObject(raw_datum, "run", run_datum);

  cJSON *edit_datum = cJSON_CreateNumber(status.edit);
  cJSON_AddItemToObject(raw_datum, "edit", edit_datum);

  cJSON *motion_datum = cJSON_CreateNumber(status.motion);
  cJSON_AddItemToObject(raw_datum, "motion", motion_datum);

  cJSON *mstb_datum = cJSON_CreateNumber(status.mstb);
  cJSON_AddItemToObject(raw_datum, "mstb", mstb_datum);

  cJSON *emergency_datum = cJSON_CreateNumber(status.emergency);
  cJSON_AddItemToObject(raw_datum, "emergency", emergency_datum);

  cJSON *alarm_datum = cJSON_CreateNumber(status.alarm);
...
int readMachinePartCount(cJSON *datum)
{
  ...
  // count
  cJSON *count_datum = cJSON_CreateNumber(param.u.ldata);
  ...
  // milliseconds
  long milliseconds = time.msec + time.minute * 60 * 1000;
  ...
  // raw minutes
  cJSON *raw_minutes_datum = cJSON_CreateNumber(time.minute);
  cJSON_AddItemToObject(raw_cycle_time_datum, "minutes", raw_minutes_datum);

  // raw milliseconds
  cJSON *raw_milliseconds_datum = cJSON_CreateNumber(time.msec);
...
int main()
{
  ...
  ret = cnc_allclibhndl3(mDeviceIP, mDevicePort, 10, &fLibHandle);
  if (ret != EW_OK)
  {
  ...
  if (readMachinePartCount(partCountDatum))
  {
    fprintf(stderr, "failed to read machine part count data!\n");
    exit(EXIT_FAILURE);
    return 1;
  }
  cJSON_AddItemToArray(updates, partCountDatum);

  cJSON *machineStatus = cJSON_CreateObject();
  if (readMachineStatus(machineStatus))
  {
    fprintf(stderr, "failed to read machine status!\n");
    exit(EXIT_FAILURE);
    return 1;
  }
  cJSON_AddItemToArray(updates, machineStatus);

  cJSON *machineToolInfo = cJSON_CreateObject();
  if (readMachineToolInfo(machineToolInfo))
  {
    fprintf(stderr, "failed to get tool info!\n");
    exit(EXIT_FAILURE);
    return 1;
  }
  cJSON_AddItemToArray(updates, machineToolInfo);

  cJSON *machineDynamic = cJSON_CreateObject();
