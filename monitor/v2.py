...
class AdapterMonitor:
    ...
    async def __status_worker(self):
        try:
            while True:
                machine_id, status, status_message = await self.status_queue.get()
                print(f'{machine_id=} {status=}')
                async with self.pool.acquire() as con:
                    async with con.transaction():
                        status_id = await con.fetchval('''
                            insert into machine_monitor_status_updates(machine_id,status,status_message) values ($1,$2,$3) returning id
                        ''', machine_id, status, status_message)
                        await con.execute('''
    ...
    async def monitor_machine(self, machine_id, machine_ip, machine_port, sleep_timeout=5):
        try:
            for i in itertools.count(0):
                await self.status_queue.put((machine_id, 'RESTARTING' if i > 0 else 'STARTING', None))
                args = [self.executable, f'{machine_id=}', f'{machine_ip=}', f'{machine_port=}']
                ...
                await self.status_queue.put((machine_id, 'FAILED' if returncode else 'STOPPED', stderr.decode() if stderr else None))
    ...
    async def __updates_worker(self):
        async with self.pool.acquire() as con:
            for row in await con.fetch('''
                select machine_id,machine_ip,machine_port from machine_monitor
            '''):
            ...
            def cb(*args):
                ...
                if d:
                    machine_ip, machine_port = d['machine_ip'], d['machine_port']
                    payload = (machine_ip, machine_port)
                else:
                    payload = None
            ...
            try:
                ...
                while True:
                    ...
                    if machine_id in self.tasks:
                        await self.status_queue.put((machine_id, 'RELOADING' if config else 'DESTROYING', None))
                        task = self.tasks.pop(machine_id)
                        task.cancel()
                        await task 
                        if not config:
                            await self.status_queue.put((machine_id, 'DESTROYED', None))
                    elif config is None:
                        # invalid condition
                        continue
                    ...
                    if config is not None:
                        ...
                        coro = self.monitor_machine(machine_id, machine_ip, machine_port)
...
if __name__ == '__main__':
    ...
    loop.run_until_complete(o.connect('postgresql://postgres:password@localhost:5432/testing'))
